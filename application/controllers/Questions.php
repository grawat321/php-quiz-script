...
class Questions extends CI_Controller
{
	...
	function view()
	{
		if($this->session->username) 		
		{
			if($this->session->type==1 || $this->session->type==2) 		
			{
				...
				{				
					$data0=$this->a->id_view('phpuncle_question',$id);
					if($data0==NULL) redirect('Questions/view');
					$data0['id']=$id;

					$this->load_view('questions/detail_view',$data0);
					return ;
				}
	
				$this->load_view('questions/view');	
				$this->session->msg = "";	
		...
		if($this->session->username) 		
		{
			if($this->session->type==1) 		
			{
				...
				{	
					...
					$this->a->update('phpuncle_question',$id,$data);

					$this->session->msg = "Question updated successfully.";
					$this->session->msg_type = "success";							
				...
				$this->load_view('questions/edit',$data0);	
			}
			else
			{
	           	 redirect('Home');
	...
	function delete($id)
	{
		if($this->session->username) 		
		{
			...
			{	
				$this->a->delete('phpuncle_question',$id);
				
				$this->session->msg = "Question deleted successfully.";
				$this->session->msg_type = "success";							
